...
func (builder *VCBuilder) PreBlindSign(secDoc *credential.Credential, nonce []byte, opts ...processor.ProcessorOpts) ([]byte, []byte, error) {
	if builder.credential == nil {
		return nil, nil, fmt.Errorf("no credential parsed")
	}
	s := builder.signatureSuite[bbsblssignature2020.SignatureType]
	if s == nil {
		return nil, nil, fmt.Errorf("expected at least one signature suit present")
	}
	return s.(*bbsblssignature2020.BBSSuite).PreBlindSign(builder.credential, secDoc, nonce, opts...)
}

func (builder *VCBuilder) BlindSign(ctxBytes []byte) ([]byte, error) {
	if builder.credential == nil {
		return nil, fmt.Errorf("no credential parsed")
	}
	s := builder.signatureSuite[bbsblssignature2020.SignatureType]
	...
	return nil, nil
	// s.(*bbsblssignature2020.BBSSuite).BlindSign(ctxBytes,builder.credential.ToMap(),)
}
